...
def calculate_fv(v, g, disp):
    return v/np.sqrt(g*np.cbrt(disp/rho))
...
def calculate_m(lcg, b, fv):
    return 0.98 + 2 * (lcg / b)**1.45 * np.e**(-2 * (fv - 0.85)) - 3 * (lcg / b) * np.e**(-3 * (fv - 0.85))


def blount_and_fox(b, vmin, vmax, nspeeds, disp, deadrise_angle, lcg, trim_step, Sea_Margin, efficiency):
    tol = trim_step / 2.5
    ...
    drag_savitsky = np.zeros(nspeeds, dtype=float)
    ...
    speed_savitsky = np.zeros(nspeeds, dtype=float)
    ...
    for i in range(nspeeds):
        ...
        if np.rad2deg(trim) > 2.0:
            trim_minimum = 2.0

        if fv >= 1:
            m = calculate_m(lcg, b, fv)

            drag[j] = calculate_drag(disp, trim_angle, dfric) * m
            dfriction[j] = dfric
            trims[j] = trim
            lift[j] = n
            ...
            cfric[j] = cf * m
            cpres[j] = calculate_cpressure(dpressure[j], rho, v_1, lb_lambda, b) * m
            ctotal[j] = calculate_ctotal(drag[j], rho, v_1, lb_lambda, b)
            speed[j] = v

            if cv >= 1:
                drag_savitsky[k] = calculate_drag(disp, trim_angle, dfric)
                speed_savitsky[k] = v
                k += 1
    ...
    cpres = cpres[:j]
    ctotal = ctotal[:j]
    drag_savitsky = drag_savitsky[:k]
    speed_savitsky = speed_savitsky[:k]
    ...
    return (drag, speed/kn_to_ms, trims, lift, dfriction, dpressure, lam, reynolds, cfric, cpres, ctotal, EKW, BKW,
            drag_savitsky, speed_savitsky/kn_to_ms)
...
if __name__ == '__main__':
    ...
    (drag, speeds, trims, lift, dfriction, dpressure, lam, rein, cfri, cpres, ctotal, EKW, BKW, drag_savitsky,
     speed_savitsky) = (
        blount_and_fox(B, Vmin, Vmax, Nspeeds, disp, Deadrise_angle, LCG, trim_step, Sea_Margin, efficiency))
