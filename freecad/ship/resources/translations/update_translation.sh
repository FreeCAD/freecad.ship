...
get_strings() {
	...
	lupdate ../ui/*.ui -verbose -ts uifiles.ts
	# Get translatable strings from Python file(s)
	# pylupdate5 ../../*.py -ts pyfiles.ts -verbose
	pylupdate6 ../../init_gui.py \
		../../resistanceAmadeo/{PlotAux,TaskPanel}.py \
		../../resistanceHoltrop/{PlotAux,TaskPanel}.py \
		../../seakeepingRAOs/TaskPanel.py \
		../../seakeepingSetMesh/TaskPanel.py \
		../../shipAreasCurve/{TaskPanel,PlotAux}.py \
		../../shipCapacityCurve/{PlotAux,TaskPanel,Tools}.py \
		../../shipCreateLoadCondition/__init__.py \
		../../shipCreateShip/{Preview,TaskPanel}.py \
		../../shipCreateTank/TaskPanel.py \
		../../shipCreateWeight/TaskPanel.py \
		../../shipGZ/{PlotAux,TaskPanel,Tools}.py \
		../../shipHydrostatics/{PlotAux,TaskPanel,Tools}.py \
		../../shipLoadExample/TaskPanel.py \
		../../shipSinkAndTrim/{TaskPanel,Tools}.py \
		../../shipUtils/Selection.py \
		../../TankInstance.py \
		../../WeightInstance.py \
		../../ShipGui.py \
		../../Instance.py \
	...
	# NOTE: The `lupdate` binary from Qt6 can handle both `.py` and `.ui` files
	# in the same command, so you will not longer need to use `pylupdate` and `lconvert`
	# to join the strings from different sources.
}
